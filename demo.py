...
def test():
    ...
    imagefile = sorted(glob.glob(os.path.join(video_dir, '*.jpg'))+glob.glob(os.path.join(video_dir,'*.png')))
    imagefiles = []
    ...
    with torch.no_grad():
        for index in range(0,len(imagefiles)):
            ...
            ## save pred_mask
            save_mask_folder = prediction_result_dir
    ...
    print("Prediction results have been saved in {}".format(save_mask_folder))
