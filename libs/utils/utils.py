...
def save_checkpoint_epoch(args, result_path, encoder, decoder, enc_opt, dec_opt, epoch, best=False):

    torch.save(encoder.state_dict(), os.path.join(result_path, 'encoder_{}.pt'.format(epoch)))
    torch.save(decoder.state_dict(), os.path.join(result_path, 'decoder_{}.pt'.format(epoch)))
    torch.save(enc_opt.state_dict(), os.path.join(result_path, 'enc_opt_{}.pt'.format(epoch)))
    torch.save(dec_opt.state_dict(), os.path.join(result_path, 'dec_opt_{}.pt'.format(epoch)))

    if best:
        torch.save(encoder.state_dict(), os.path.join(result_path, 'encoder.pt'))
        torch.save(decoder.state_dict(), os.path.join(result_path, 'decoder.pt'))
        torch.save(enc_opt.state_dict(), os.path.join(result_path, 'enc_opt.pt'))
        torch.save(dec_opt.state_dict(), os.path.join(result_path, 'dec_opt.pt'))

    pickle.dump(args, open(os.path.join(result_path, 'args.pkl'), 'wb'))
...
def load_checkpoint_epoch(model_name, epoch, use_gpu=True, load_opt=True):
    if use_gpu:
        ...
        if load_opt:
            enc_opt_dict = torch.load(os.path.join(model_name, 'enc_opt_{}.pt'.format(epoch)))
            dec_opt_dict = torch.load(os.path.join( model_name, 'dec_opt_{}.pt'.format(epoch)))
    else:
        encoder_dict = torch.load(os.path.join( model_name, 'encoder_{}.pt'.format(epoch)),
